...
impl MetricsCollector {
    ...
    pub fn new(config: MetricsCollectorConfig) -> Self {
        Self::new_with_node_id(config, None)
    }
    
    /// Create a new metrics collector with a specific node ID
    pub fn new_with_node_id(config: MetricsCollectorConfig, node_id: Option<String>) -> Self {
        let node_id = node_id.unwrap_or_else(|| {
            // Generate a unique node ID based on process ID and timestamp
            let process_id = std::process::id();
            let timestamp = current_timestamp_ms();
            // Use a simple random component for additional uniqueness
            let random_component = rand::random::<u32>();
            format!("node-{}-{}-{}", process_id, timestamp, random_component)
        })
        Self {
            node_id,
    ...
    pub async fn generate_report(&self) -> Result<MetricsSnapshot> {
        // Consolidate atomic counters into the RwLock-protected structs
        {
            ...
            // Update node counts from node metrics
            let total_nodes = self.node_metrics.len();
            let connected_nodes = self.node_metrics.iter()
                .filter(|entry| matches!(entry.value().status, NodeStatus::Online))
                .count();
            ...
            network_metrics.disconnected_nodes = disconnected_nodes;
            
            // Calculate connectivity as ratio of connected to total nodes
            ...
            // Estimate average coordination number (simplified)
            network_metrics.avg_coordination_number = if connected_nodes > 0 {
                // Assume each node is connected to log(n) other nodes on average
                (connected_nodes as f64).ln().max(1.0)
            } else {
                0.0
            };
            
            // Estimate diameter (simplified)
            network_metrics.diameter_estimate = if connected_nodes > 1 {
                (connected_nodes as f64).log2().ceil() as u32
            } else {
                0
            };
        }

        {
            ...
            // Calculate averages for gossip metrics
            ...
            // For running averages, we could maintain separate counters
            // For now, use simple approach with current values
            epidemic_metrics.avg_gossip_fanout = if epidemic_metrics.gossip_rounds > 0 {
                current_fanout as f64 / epidemic_metrics.gossip_rounds as f64
            } else {
                current_fanout as f64
            };
            
            epidemic_metrics.avg_entries_per_gossip = if epidemic_metrics.gossip_rounds > 0 {
                current_entries as f64 / epidemic_metrics.gossip_rounds as f64
            } else {
                current_entries as f64
            };
            
            // Calculate other averages based on operation metrics
            if let Some(gossip_send_metrics) = self.operation_metrics.get(&OperationType::GossipSend) {
                epidemic_metrics.avg_anti_entropy_time_ms = gossip_send_metrics.average_latency_us() as u64 / 1000;
                epidemic_metrics.avg_convergence_time_ms = gossip_send_metrics.average_latency_us() as u64 / 1000;
            }
            
            // Update propagation counts
            epidemic_metrics.total_updates_propagated = self.total_messages_sent.load(Ordering::Relaxed);
        }
        ...
        // Collect top N node metrics by total operations
        let mut node_metrics_vec: Vec<NodeMetrics> = self.node_metrics
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
        node_metrics_vec.sort_by(|a, b| b.total_operations.cmp(&a.total_operations));
        let top_node_metrics = node_metrics_vec.into_iter()
            .take(self.config.max_node_metrics)
            .collect();
        
        // Collect top N key metrics by total operations
        let mut key_metrics_vec: Vec<KeyMetrics> = self.key_metrics
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
        key_metrics_vec.sort_by(|a, b| b.total_operations.cmp(&a.total_operations));
        let top_key_metrics = key_metrics_vec.into_iter()
            .take(self.config.max_key_metrics)
            .collect();
