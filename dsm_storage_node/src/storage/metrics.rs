...
impl MetricsCollector {
    ...
    pub fn new(config: MetricsCollectorConfig) -> Self {
        // Obtain node_id properly from config or generate a unique one
        let node_id = std::env::var("DSM_NODE_ID")
            .unwrap_or_else(|_| format!("node_{}", uuid::Uuid::new_v4()));
        Self {
            node_id,
    ...
    pub async fn generate_report(&self) -> Result<MetricsSnapshot> {
        // Consolidate atomic counters into the RwLock-protected structs
        {
            ...
            // Update other network metrics like node counts, connectivity etc.
            let connected_nodes = self.node_metrics.len();
            network_metrics.connected_nodes = connected_nodes;
            network_metrics.total_nodes = connected_nodes + network_metrics.disconnected_nodes;

            // Calculate connectivity as percentage of successful connections
            if network_metrics.total_nodes > 0 {
                network_metrics.connectivity =
                    connected_nodes as f64 / network_metrics.total_nodes as f64;
            }

            // Update coordination number (average connections per node)
            if connected_nodes > 0 {
                network_metrics.avg_coordination_number = 2.0; // Simple approximation
            }
        }

        {
            ...
            // Calculate averages like avg_gossip_fanout
            ...
            if epidemic_metrics.gossip_rounds > 0 {
                ...
                epidemic_metrics.avg_entries_per_gossip =
                    current_entries as f64 / epidemic_metrics.gossip_rounds as f64;
            }

            // Update convergence time estimate (simplified)
            epidemic_metrics.avg_convergence_time_ms = 500; // Default estimate
        }
        ...
        // Collect region, node, key metrics (potentially selecting top N)
        let _region_metrics_map: HashMap<_, _> = self
            .region_metrics
            .iter()
            .map(|entry| (entry.key().clone(), entry.value().clone()))
            .collect();

        // Get top N node metrics sorted by total operations
        let mut top_nodes: Vec<_> = self
            .node_metrics
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
        top_nodes.sort_by(|a, b| b.total_operations.cmp(&a.total_operations));
        top_nodes.truncate(10); // Top 10 nodes

        // Get top N key metrics sorted by total operations
        let mut top_keys: Vec<_> = self
            .key_metrics
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
        top_keys.sort_by(|a, b| b.total_operations.cmp(&a.total_operations));
        top_keys.truncate(10); // Top 10 keys
