...
run_test "JNI bindings file exists" \
    "check_file_exists 'dsm_client/decentralized_state_machine/dsm_sdk/src/sdk/jni_bindings.rs' 'JNI bindings Rust file'"
...
if [ -d "dsm_client/decentralized_state_machine/dsm_sdk" ]; then
    run_test "Rust SDK compilation" \
        "test_compilation 'dsm_client/decentralized_state_machine/dsm_sdk' 'cargo check --quiet' 'Rust SDK with JNI bindings'"
fi
